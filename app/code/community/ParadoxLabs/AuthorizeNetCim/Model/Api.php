...
//								<lineItems>' . $this->getLineItems() . '</lineItems>
		...
		$this->xml = '<?xml version="1.0" encoding="utf-8"?>
					<getHostedProfilePageRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
						...
						<hostedProfileSettings>
							...
							// <setting>
							// 	<settingName>hostedProfileHeadingBgColor</settingName>
							// 	<settingValue>false</settingValue>
							// </setting>
		$this->xml .= '
							<setting>
								<settingName>hostedProfileIFrameCommunicatorUrl</settingName>
								<settingValue>' . Mage::getUrl( 'authnetcim/ajax/communicator', array( '_secure' => true ) ) . '</settingValue>
							</setting>
						</hostedProfileSettings>
	...
	private function parseResults()
	{
		...
		$this->token 			= isset( $xml->token ) ? (string) $xml->token : '';
		$this->validation		= isset( $xml->validationDirectResponse ) ? (string) $xml->validationDirectResponse : '';
		$this->profileId		= isset( $xml->customerProfileId ) ? (int) $xml->customerProfileId : 0;
		$this->addressId		= isset( $xml->customerAddressId ) ? (int) $xml->customerAddressId : 0;
		$this->paymentProfileId = isset( $xml->customerPaymentProfileId ) ? (int) $xml->customerPaymentProfileId : 0;
		$this->results			= isset( $xml->directResponse ) ? explode( $this->delimiter, $this->directResponse ) : array();
	}
	...
	{
		return isset($this->results[10]) ? $this->results[10] : '';
	}
