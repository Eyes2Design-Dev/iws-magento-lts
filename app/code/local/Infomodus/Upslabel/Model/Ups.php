...
<Height>" . Mage::getStoreConfig('upslabel/dimansion_' . $pv['dimansion_id'] . '/height') . "</Height>";
      ...
      <PackageServiceOptions>";
        ...
        if ($this->shiptoCountryCode != 'US' && $this->shiptoCountryCode != 'PR' && $this->shiptoCountryCode != 'CA' && $this->shipfromCountryCode != 'US' && $this->shipfromCountryCode != 'PR' && $this->shipfromCountryCode != 'CA') {
            if ($this->adult > 0) {
                $data .= "<DeliveryConfirmation><DCISType>" . ($this->adult - 1) . "</DCISType></DeliveryConfirmation>";
            }
        ...
        if ($this->qvn == 1) {
            foreach ($this->qvn_code AS $qvncode) {
                $data .= "<Notification>
            <NotificationCode>" . $this->qvn_code . "</NotificationCode>
            <EMailMessage>";
...
<Height>" . Mage::getStoreConfig('upslabel/dimansion_' . $pv['dimansion_id'] . '/height') . "</Height>";
    ...
    public function deleteLabel($trnum)
    {
        $path_xml = Mage::getBaseDir('media') . DS . 'upslabel' . DS . "test_xml" . DS;
        $cie = 'wwwcie';
        $testing = Mage::getStoreConfig('upslabel/testmode/testing');
...
<Password>" . $this->Password . "</Password>
    ...
    ";
        ...
        $ch = curl_init('https://' . $cie . '.ups.com/webservices/Pickup');
        curl_setopt($ch, CURLOPT_HEADER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_TIMEOUT, 60);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	...
	<envr:Header>
        ...
        $file = file_put_contents($path_xml . "PickupCancelRequest.xml", $data);
